...
const enrollCourse = async (req, res) => {
  ...
  try {
    ...
    if (!course.enrollmentPassword) {
      return res
        .status(400)
        .json({ message: "Enrollment key not set for this course" });
    }
...
const markAttendance = async (req, res) => {
  ...
  try {
    ...
    if (
      !session ||
      session.course.toString() !== courseId ||
...
const getMyCourses = async (req, res) => {
  try {
    ...
    // For each course compute attendance stats for this student
    const result = [];
    for (const course of courses) {
      ...
      result.push({
        ...
        code: course.code,
        name: course.name,
        lecturer: course.lecturer ? course.lecturer.name : "",
